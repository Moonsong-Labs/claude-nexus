...
test.describe('Dark Mode Component Tests', () => {
  ...
  test('pagination controls should be visible in dark mode', async ({ page }) => {
    ...
    if (paginationExists) {
      // Check background
      const linkBg = await paginationLink.evaluate(el => window.getComputedStyle(el).backgroundColor)
      ...
      // Check border
      const linkBorder = await paginationLink.evaluate(el => window.getComputedStyle(el).borderColor)
  ...
  test('message components should have proper colors in dark mode', async ({ page }) => {
    ...
    if (messageExists) {
      const messageBg = await messageContent.evaluate(el => window.getComputedStyle(el).backgroundColor)
