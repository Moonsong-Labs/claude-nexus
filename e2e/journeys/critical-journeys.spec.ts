...
test.describe('@journey Critical User Journeys', () => {
  ...
  test('Journey 1: View request details flow', async ({ browser }) => {
    ...
    try {
      ...
      const requestsLink = page.locator('a[href="/requests"], [data-testid="requests-link"]').first();
  ...
  test('Journey 2: Check token usage and analytics', async ({ browser }) => {
    ...
    try {
      ...
      const tokenLink = page.locator('a[href="/token-usage"], [data-testid="token-usage-link"]').first();
      ...
      if (await timeSelector.isVisible()) {
        ...
        await page.waitForResponse(resp => resp.url().includes('/token-usage') || resp.url().includes('/api'), { timeout: 2000 }).catch(() => {});
      }
  ...
  test('Journey 3: Navigate conversation tree', async ({ browser }) => {
    ...
    try {
      ...
      if (await conversationLink.isVisible()) {
        ...
        await expect(page.locator('.conversation-tree, [data-testid="conversation-tree"], svg')).toBeVisible();
        ...
        if (await firstNode.isEnabled()) {
          ...
          await page.waitForSelector('.tooltip, .detail-panel, [data-testid="node-detail"]', { timeout: 1000 }).catch(() => {});
        }
      }
  ...
  test('Journey 5: View AI analysis results', async ({ browser }) => {
    ...
    try {
      ...
      const analysisIndicator = page.locator('.analysis-badge, [data-testid="has-analysis"], text=/analysis/i').first();
      if (await analysisIndicator.isVisible()) {
        // Click on the request with analysis
        const requestRow = analysisIndicator.locator('xpath=ancestor::tr | ancestor::*[@class="request-item"]').first();
        ...
        const analysisPanel = page.locator('.analysis-panel, [data-testid="analysis-panel"], text=/analysis/i').first();
        await expect(analysisPanel).toBeVisible();

        // Step 5: Verify analysis content loaded
        const analysisContent = page.locator('.analysis-content, [data-testid="analysis-content"]').first();
