...
describe('Sub-task Detection', () => {
  it('should detect Task tool invocations in response body', () => {
    ...
    expect(taskInvocations[0]).toEqual({
      ...
      input: {
        ...
        prompt: expect.stringContaining('I need you to analyze the Claude Nexus Proxy repository structure')
      }
    })
  })
  ...
  it('should not detect Task invocations in responses without Task tools', () => {
    ...
    const taskInvocations = writer.findTaskToolInvocations(subtaskRequestSample.response?.body || {})
