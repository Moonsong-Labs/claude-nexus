...
describe('Special Conversation Linking Cases', () => {
  describe('Conversation Summarization', () => {
    it('should detect summarization system prompt', () => {
      ...
      const systemWithSummarization = 'You are a helpful AI assistant tasked with summarizing conversations. Please provide a concise summary.'
      const systemNormal = 'You are a helpful AI assistant.'
      
      // Both should produce same message hash
      const { currentMessageHash: hash1, systemHash: sysHash1 } = extractMessageHashes(messages, systemWithSummarization)
      const { currentMessageHash: hash2, systemHash: sysHash2 } = extractMessageHashes(messages, systemNormal)
...
Please continue the conversation from where we left it off without asking the user any further questions. Continue with the last task that you were asked to work on.`
      ...
      const messages: ClaudeMessage[] = [
        { 
          role: 'user', 
          content: [
            { type: 'text', text: continuationMessage }
          ]
        },
      ]
...
Please continue the conversation from where we left it off without asking the user any further questions.`
  ...
  describe('Branch ID Generation', () => {
    it('should generate compact branch ID from timestamp', () => {
      ...
      const branchId = `compact_${timestamp.toISOString().replace(/[^0-9]/g, '').substring(8, 14)}`
