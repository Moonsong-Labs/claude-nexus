...
async function parseMessage(msg: any, timestamp?: Date): Promise<ParsedMessage> {
  ...
  const htmlContent = sanitizeHtml(dirtyHtml, {
    allowedTags: sanitizeHtml.defaults.allowedTags.concat(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'pre', 'code']),
    allowedAttributes: {
...
const DEFAULT_INPUT_COST_PER_MTOK = 15.00;  // $15 per million tokens (Opus default)
const DEFAULT_OUTPUT_COST_PER_MTOK = 75.00; // $75 per million tokens (Opus default)
...
export function calculateCost(inputTokens: number, outputTokens: number): {
  ...
  const inputCostPerMtok = parseFloat(process.env.ANTHROPIC_INPUT_COST_PER_MTOK || '') || DEFAULT_INPUT_COST_PER_MTOK;
  const outputCostPerMtok = parseFloat(process.env.ANTHROPIC_OUTPUT_COST_PER_MTOK || '') || DEFAULT_OUTPUT_COST_PER_MTOK;
