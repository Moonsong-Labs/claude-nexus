...
dashboardRoutes.get('/', async (c) => {
  ...
  if (pool) {
    try {
      const domainsResult = await pool.query('SELECT DISTINCT domain FROM api_requests ORDER BY domain')
  ...
  const content = html`
    ...
    <div class="mb-6">
      ...
      <select onchange="window.location.href = '/dashboard' + (this.value ? '?domain=' + this.value : '')" style="margin-left: 0.5rem;">
        ...
        ${raw(domains.map(d => `<option value="${d}" ${domain === d ? 'selected' : ''}>${d}</option>`).join(''))}
      </select>
    </div>
    ...
    <div class="section">
      <div class="section-header">
        Recent Requests
        <a href="/dashboard" class="btn btn-secondary" style="float: right; font-size: 0.75rem; padding: 0.25rem 0.75rem;">Refresh</a>
      </div>
      <div class="section-content">
        ${stats.recentRequests.length === 0 ? html`
        ...
        ` : html`
          <table>
            ...
            <tbody>
              ${raw(stats.recentRequests.map(req => `
                <tr>
