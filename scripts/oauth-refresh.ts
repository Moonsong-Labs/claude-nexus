...
async function refreshOAuthToken() {
  ...
  if (!credentialPath) {
    ...
    console.error('Example: bun run scripts/oauth-refresh.ts credentials/example.com.credentials.json')
    console.error('\nOptions:')
  ...
  try {
    ...
    try {
      ...
      console.log(`- Refresh Token: ${newOAuth.refreshToken ? newOAuth.refreshToken.substring(0, 20) + '...' : 'reused existing'}`)
