...
The Analysis API provides endpoints for generating and managing AI-powered analyses of conversations.
...
Creates a new analysis for a conversation branch. Returns 409 if an active analysis already exists.
...
Retrieves the latest analysis for a conversation branch.
...
Creates a new analysis for a conversation branch that already has an existing analysis.
...
- `failed` - Analysis generation failed (can be retried)

For detailed documentation, see [Conversation Analysis API Documentation](/docs/api/conversation-analysis-api.md).
