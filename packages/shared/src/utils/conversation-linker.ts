...
const COMPACT_CONVERSATION_PREFIX = 'This session is being continued from a previous conversation that ran out of context'
...
export class ConversationLinker {
  ...
  async linkConversation(request: LinkingRequest): Promise<LinkingResult> {
    ...
    if (systemHash) {
      const exactMatches = await this.findParentByHash(domain, parentMessageHash, systemHash, requestId)
    ...
    if (!parent && this.isSummarizationRequest(systemPromptStr)) {
      const summarizationMatches = await this.findParentByHash(domain, parentMessageHash, null, requestId)
      parent = this.selectBestParent(summarizationMatches)
    }

    // Priority iii: Fallback - match by message hash only
    if (!parent) {
      const fallbackMatches = await this.findParentByHash(domain, parentMessageHash, null, requestId)
      parent = this.selectBestParent(fallbackMatches)
    }

    if (parent) {
      // Check if this creates a branch
      const existingChildren = await this.findChildrenOfParent(domain, parent.current_message_hash, requestId)
  ...
  private computeMessageHash(messages: ClaudeMessage[]): string {
    try {
      ...
      throw new Error(`Failed to compute message hash: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  private deduplicateToolItems(content: ClaudeContent[]): ClaudeContent[] {
    ...
    return content.filter(item => {
      if (item.type === 'tool_use' && item.id) {
        if (seenToolUseIds.has(item.id)) return false
        seenToolUseIds.add(item.id)
      } else if (item.type === 'tool_result' && item.tool_use_id) {
        if (seenToolResultIds.has(item.tool_use_id)) return false
  ...
  private serializeContentItems(content: ClaudeContent[]): string {
    return content
      .map((item, index) => this.serializeContentItem(item, index))
      .join('\n')
  }
  ...
  private computeParentHash(messages: ClaudeMessage[]): string {
    ...
    if (messages.length < MIN_MESSAGES_FOR_PARENT_HASH) {
      throw new Error(`Cannot compute parent hash for less than ${MIN_MESSAGES_FOR_PARENT_HASH} messages`)
    }
  ...
  private detectCompactConversation(message: ClaudeMessage): CompactInfo | null {
    try {
      ...
      if (textContent && textContent.includes(COMPACT_CONVERSATION_PREFIX)) {
        ...
        if (summaryStart > -1) {
          const summaryContent = this.extractSummaryContent(textContent, summaryStart + SUMMARY_MARKER.length)
          return {
            isCompact: true,
            summaryContent
          }
  ...
  private async findCompactParent(domain: string, summaryContent: string): Promise<ParentRequest | null> {
    try {
  ...
  private isSummarizationRequest(systemPrompt?: string | { type: 'text'; text: string; cache_control?: { type: 'ephemeral' } }[]): boolean {
